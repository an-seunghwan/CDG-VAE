...
def train(dataloader, model, B_est, config, optimizer):
    ...
    for batch in iter(dataloader):
        ...
        recon = F.mse_loss(model(torch.matmul(batch, B_est)), batch)
...
def main():
    ...
    train_imgs = os.listdir('./utils/causal_data/pendulum_orig/train')
    ...
    wandb.run.summary['Is DAG?'] = is_dag(B_est)
    fig = viz_heatmap(B_est, size=(5, 4))
