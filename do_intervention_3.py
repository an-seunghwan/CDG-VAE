...
def main():
    ...
    # B = model.W * model.mask
    # I_B_inv = torch.inverse(model.I - B)
    ...
    # xhat = model.decoder(torch.cat(latent, dim=1))
    # xhat = xhat.view(-1, model.config["image_size"], model.config["image_size"], 3)
    
    # """Alignment"""
    # mean_ = mean.view(-1, model.config["node_dim"], model.config["node"]).contiguous() # deterministic part
