...
class VAE(nn.Module):
    ...
    def forward(self, input, running=True):
        ...
        """Alignment Scaling""" # [batch, node_dim] x node
        stat_mean = [x.mean(axis=0) for x in align_latent]
        stat_std = [x.std(axis=0) for x in align_latent]
        if running:
            align_latent = [(x - m) / s for x, m, s in zip(align_latent, stat_mean, stat_std)]
            # update
            ...
            align_latent = [(x - m) / s for x, m, s in zip(align_latent, self.running_mean, self.running_std)]
