...
#     file = [x for x in se_list if x.startswith(n)]
...
#                 tmp_result.append(np.array([float(x.split(': ')[-1]) * 100 for x in tmp]))
#         mean = np.array(tmp_result).mean(axis=0)
#         std = np.array(tmp_result).std(axis=0)
#         se_result['_'.join(f.split('_')[0:-1])] = [mean, std]
#     else:
#         for i in range(2):
...
#             se_result['_'.join(f.split('_')[0:-1])] = [mean, std]
# #%%
# with open('./assets/sample_efficiency.txt', 'w') as f:
