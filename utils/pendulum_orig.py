...
import numpy as np
import pandas as pd 
if not os.path.exists('./causal_data/pendulum_orig/'): 
  os.makedirs('./causal_data/pendulum_orig/train/')
  os.makedirs('./causal_data/pendulum_orig/test/')
...
for i in range(-40,44):#pendulum
    for j in range(60,148):#light
        ...
        plt.axis('off')
        if count == 4:
          plt.savefig('./causal_data/pendulum_orig/test/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
          count = 0
        else:
          plt.savefig('./causal_data/pendulum_orig/train/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
        plt.clf()
        count += 1
#%%